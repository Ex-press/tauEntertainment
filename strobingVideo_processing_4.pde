...
void helpText(int xPos, int yPos)
{
  fill(255);
  pushMatrix();
  translate(xPos, yPos);
...
void draw() {
  try {
    ...
    movieHeight=movieWidth*2/3;    //the first couple frames, the movie hasn't initialized yet, so it throws an error if I try to access movie.width   -- this just assigns a default value to movieHeight
  }
  if (frameCount % 2 ==0)
    fill(0);
  else
